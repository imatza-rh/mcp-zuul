...
async def compare_builds(
    ...
    """Compare two builds side-by-side — highlights differences in result, timing, and nodeset."""
    ...
    return (
        ...
        "2. If one passed and one failed, identify what's different\n"
        "3. Check if it's a flaky test, infrastructure change, or code regression\n"
        "4. Use `get_build_failures` on the failed build for detailed task-level analysis"
    )
